...
**ตอนนี้มีโหมด 1-9 แล้ว** (โหมดถัดไป = 10):
...
| 9 | หน้า search modal | stock | 4 endpoints (recommend/master/autocomplete/youtube) |
...
3. save → `npm start` → เมนูมีโหมดใหม่ให้เลือกอัตโนมัติ

API เส้นจริงที่ยิง = `${API_BASE}/${type}/latest/${column}?limit=5&lang=th`

> 💡 ถ้าให้ Claude ทำให้: มันจะเด้ง popup ถามข้อมูล 4 ค่านี้ก่อน แล้วเพิ่มลง `config.js` ให้เอง
